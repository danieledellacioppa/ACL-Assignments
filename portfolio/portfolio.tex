...
\subsubsection{Infinite wait loop}
Checking if the input BufferedReader is ready to be read can be done using a built-in method of the class BufferedReader which the ready( ) function. Let \textbf{in} be an instance of the BufferedReader class. Then the instruction to use will be:

\[in.ready()\]

Performing a readLine( ) on a input Buffered reader which is actually empty makes the thread doing the readLine() to wait indefinitely for something to happen on that Buffer.
If nothing ever happens the Thread will wait for ever for something readable to be present in the Buffer. This is avoided by checking the readiness of the Buffer first. 
...
\begin{lstlisting}
                if(in.ready())
                {
                   debugConsoleSimpleClient.append("the input buffer seems to be ready\n");
                   line = in.readLine();
                }
                   else
                   line=null;
