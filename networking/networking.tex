...
The term \emph{wiring} can be deceiving here as there are literally no wires involved to realize the connection. In Cantonese is probably much clearer (Figure 1.2).
...
%\vspace{-10pt}
\caption{The TCPServer process has two sockets}\label{tcpsocket}
\end{wrapfigure}
...
There's a debate on which one is vertical and which is horizontal so that point won't be discussed in depth in this document for the time being but for the sake of quoting quora.com:

\medskip

\textbf{In the OSI model, there are no intermediate nodes between transport layer or above layers, hence these layers communicate directly with each other, hence horizontal approch.}

\medskip

\textbf{While in TCP/IP, each layer is traversed starting from application layer to transport layer then internet layer then goes to the host and then the network layer, and then msg is sent. Meaning data travels each layer, accessing their functionality, only then it is sent, hence vertical approch.}

\medskip

But to me it doesn't really make sense the whole matter. I mean what's the point? If a question like this comes out in the exam I'll just have a random guess

\section{Switched Local Area Networks}

\begin{wrapfigure}{R}{11cm}
\centering
\includegraphics[width=10cm]{./switched-lan.PNG}
%\vspace{-10pt}
\caption{An institutional network connected together by four switches}\label{wrap-fig:5}
\end{wrapfigure}


Let's do now talk about Switched local networks. Figure 2.4 shows a switched local network connecting two servers, three departments and a router using four switches. Because these switches operate at the link layer (layer-2 if you recall Figure 2.2), they switch link-layer frames (rather than network-layer datagrams), hence don’t recognize network-layer addresses, thus don’t use routing algorithms like for example OSPF to determine paths through the network of layer-2 switches. Rather than using IP addresses, we will soon see that they use link-layer addresses to forward link-layer frames through the network of switches. We shall begin our study of switched LANs by first covering linklayer addressing (Section 6.4.1). We then take a close look to the famous Ethernet protocol (Section 6.4.2). Once we've seen the link-layer addressing and Ethernet, we’ll look at how link-layer switches operate (Section 6.4.3), and then see (Section 6.4.4) how these switches are often used to build large-scale LANs.
...
many countries by many companies. How does a company manufacturing adapters in Taiwan make sure that it is using different addresses from a company manufacturing 
