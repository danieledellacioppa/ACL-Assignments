...
LAN, which stands for \textbf{local area network}, and WAN, which stands for \textbf{wide area network}, are two types of networks and \emph{as the naming conventions suggest}, LANs are for more localized networking\footnote{\label{LAN}in a home,business,school,etc.}
...
\AddToShipoutPictureBG*{%
  \AtPageUpperLeft{%
    \raisebox{-\height}{%
      \includegraphics[width=\paperwidth]{cloud.jpg}%
    }%
  }
}
...
\noindent What Cloud computing delivers is a set of different services like data storage, servers, databases, networking, and software.Rather than keeping files on a proprietary hard drive or local storage device, cloud-based storage makes it possible to save them to a remote database. As long as an electronic device has access to the web, it has access to the data and the software programs to run it.

\clearpage

\section{Introduction}
%It's meant for storage, virtual CPUs, and services like servers and computers. 
Let's differentiate between Private Public and Hybrid
...
\subsection{Switching between providers}
...
\AddToShipoutPictureBG*{%
  \AtPageUpperLeft{%
    \raisebox{-\height}{%
      \includegraphics[width=\paperwidth]{firewall.jpg}%
    }%
  }
}

\section{What is it?} 
A firewall is a combination of \textbf{hardware} and \textbf{software} that isolates an organization’s internal network from the Internet at large, allowing some packets to pass and blocking others. A firewall allows a network administrator to control access between the outside world and resources within the administered network by managing the traffic
flow to and from these resources. A firewall has three goals:


\begin{wrapfigure}[12]{R}{10cm}
\centering
\includegraphics[width=10cm]{./firewall-placement.png}
%\vspace{-10pt}
\caption{Firewall placement between the administered network and the outside world}\label{fig:firewall-placement}
\end{wrapfigure}



\begin{itemize}
\item{\emph{All traffic from outside to inside, and vice versa, passes through the firewall}. Figure \ref{fig:firewall-placement} shows a firewall, sitting at the boundary between the administered network and the rest of the Internet. While large organizations may use multiple levels of firewalls or distributed firewalls [Skoudis 2006], locating a firewall at a single access point to the network, as shown in Figure \ref{fig:firewall-placement}, makes it easier to manage and enforce a security-access policy.}
\item{\emph{Only authorized traffic, as defined by the local security policy, will be allowed to pass}. With all traffic entering and leaving the institutional network passing through the firewall, the firewall can restrict access to authorized traffic.}
\item{\emph{The firewall itself is immune to penetration}. The firewall itself is a device connected to the network. If not designed or installed properly, it can be compromised, in which case it provides only a false sense of security (which is worse than no firewall at all!).}
\end{itemize}

\noindent Cisco and Check Point are two of the leading firewall vendors today. You can also easily create a firewall (packet filter) from a Linux box using iptables (public-domain software that is normally shipped with Linux). Furthermore, firewalls are now frequently implemented in routers and controlled remotely. 

%Firewalls can be classified in three categories: \textbf{traditional packet filters}, \textbf{stateful filters}, and \textbf{application gateways}. We’ll cover each of these in turn in the following subsections.
...
On a PlayStation the DMZ host It's a single IP where firewall rules do not apply. Can be useful to set a videogaming console on DMZ to allow some services to run or the firewall will stop them.

 
%\section{Firewall Types}
%You need both Hardware and Software because threats comes from everywhere. %IT k Funde
%\subsection{Hardware}
...
We can differentiate Firewall depending on the function they provide.

\subsection{Packet-filtering Firewalls}
%only packet level but not that secure. cheap, very effective.
...
For a company of 30 employees the only thing I would not consider purchasing an internal firewall since the amount of threats we can have from the inside is comparatively miniscule. I'd still use Proxy Firewalls though to protect the identity of the employees, although might reduce speed in communication. One Next Gen Firewall should be fine since it mixes all the firewall types together. As i said is a relatively small company so one external Firewall mixing all of them should be reasonable in terms of price. Internal could be useful against man-in-the-middle
...
\section{What it is?}
%A process to make sure computer systems operate as desired
%if we get an audit we can see from one pc all the software installed on every computer
...
\subsection{why manage configuration?}
...
\begin{itemize}
\item{Classify and manage systems by groups and subgroups.} 
\item{Centrally modify base configurations.}
\item{Roll out new settings to all applicable systems.}
\item{Automate system identification, patches, and updates}
\item{Identify outdated, poor performing, and noncompliant configurations.}
\item{Prioritize actions.}
\item{Access and apply prescriptive remediation.}
\end{itemize}

\subsection{what can be managed?}
...
\subsection{benefits of automating}
...
%https://www.redhat.com/en/topics/management/what-is-an-soe

\subsection{what can go wrong with misconfiguration}

internal audit with questions trying to find out what happened. Something that could have been automated but it's still being done manually.

audit question are like:
what software patches have you applied?

%would a spreadsheet work for a big company?

\subsection{benefits of a good config management tools}
simplicity, centralized based management
